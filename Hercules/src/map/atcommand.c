...
#define RS_CMD				ACMD
#define RS_AT				"@"
#define RS_INFO_TYPE		struct AtCommandInfo *
#define RS_INFO				info
#define RS_msg_txt(id)		msg_txt(id)
#define RS_SUCCESS			true
#define RS_FAIL				false
#define RS_can_use_cmd(cm)	pc->can_use_command(sd, "@" cm)
#define RS_unban(cmd)		atcommand_hardware_unban(fd, sd, "@" cmd, (char *)atcmd_player_name, info)
#define RS_time				timer->gettick()

#define Command_Name		info->command
#define sql_handle			map->mysql_handle

#define Sql_NextRow			SQL->NextRow
#define Sql_EscapeStringLen	SQL->EscapeStringLen
#define Sql_Query			SQL->Query
#define Sql_GetData			SQL->GetData
#define Sql_FreeResult		SQL->FreeResult
#define Sql_AffectedRows	SQL->AffectedRows

#define map_nick2sd			map->nick2sd
#define map_charid2sd		map->charid2sd
#define clif_displaymessage	clif->message
#define clif_GM_kick		clif->GM_kick
#define AT_CMD_NAME			""
#endif

#ifdef RS_rA
#define RS_CMD				ACMD_FUNC
#define RS_AT				""
#define RS_INFO_TYPE		const char *
#define RS_INFO				command
#define RS_msg_txt(id)		msg_txt(sd, id)
#define RS_SUCCESS			0
#define RS_FAIL				-1
#define RS_can_use_cmd(cm)	pc_can_use_command(sd, cm, COMMAND_ATCOMMAND)
#define RS_unban(cmd)		atcommand_hardware_unban(fd, sd, "@" cmd, (char *)atcmd_player_name)
#define RS_time				gettick()

#define Command_Name		command
#define sql_handle			mmysql_handle
#define AT_CMD_NAME			"@"
#endif

bool hardware_query_sub(Sql* handle, const char * query, const char * str, char * output) {
	size_t len;
	char * esc_str;
	char * data;
	...
	Sql_EscapeStringLen(sql_handle, esc_str, str, len);

	if (SQL_ERROR == Sql_Query(handle, query, esc_str)) {
...
/*==========================================
 * ragnaunban command (usage: ragnaunban <player_name>)
 *------------------------------------------*/
RS_CMD(hardware_unban)
{
	char account_id[21];
	char * data;
	char mac[240];
	char cpu[240];
	char diskdrive[240];
	char motherboard[240];
	uint64 numRows;
	int group_id = 99;
	int matches = 0;
	
	if (sd != NULL) {
		group_id = pc_get_group_level(sd);
	}
	
	memset(atcmd_player_name, '\0', sizeof(atcmd_player_name));

	if (!message || !*message || !(
		sscanf(message, "\"%23[^\"]\"", atcmd_player_name) > 0 ||
		sscanf(message, "%23[^\n]", atcmd_player_name) > 0
		)) {
		char output[100];
		sprintf(output, "usage: " RS_AT "%s <char name>", Command_Name);
		clif_displaymessage(fd, "Please enter a player name");
		clif_displaymessage(fd, output);
		return RS_FAIL;
	}

	if (!hardware_query_sub(sql_handle, "SELECT `account_id` FROM `char` WHERE `name` = '%s'", atcmd_player_name, account_id) || account_id == NULL) {
		clif_displaymessage(fd, RS_msg_txt(3)); // Character not found.
		return RS_FAIL;
	}

	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `last_mac`,`last_cpu`,`last_drive`,`last_motherboard` FROM `login` WHERE `account_id` = '%s' AND `group_id` < '%d'", account_id, group_id)) {
		Sql_ShowDebug(sql_handle);
		return RS_FAIL;
	}

	if (SQL_SUCCESS != Sql_NextRow(sql_handle)) {
		clif_displaymessage(fd, "Account not found or GM level too low.");
		Sql_FreeResult(sql_handle);
		return RS_FAIL;
	}

	Sql_GetData(sql_handle, 0, &data, NULL);
	safestrncpy(mac, data, sizeof(mac));
	Sql_GetData(sql_handle, 1, &data, NULL);
	safestrncpy(cpu, data, sizeof(cpu));
	Sql_GetData(sql_handle, 2, &data, NULL);
	safestrncpy(diskdrive, data, sizeof(diskdrive));
	Sql_GetData(sql_handle, 3, &data, NULL);
	safestrncpy(motherboard, data, sizeof(motherboard));
	Sql_FreeResult(sql_handle);

	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT count(*) FROM `ragnashield_banlist` WHERE (`mac` = '%s' OR (`cpu_id` = '%s' AND `drive_id` = '%s' AND `motherboard_id` = '%s')) AND `issuer_group_id` > '%d'", mac, cpu, diskdrive, motherboard, group_id)) {
		Sql_ShowDebug(sql_handle);
		return RS_FAIL;
	}
	...
	if (matches > 0) {
		clif_displaymessage(fd, "The ban has been made by a higher GM.");
		clif_displaymessage(fd, "The hardware ban info will not be updated.");
	}
	else {
		if (SQL_ERROR == Sql_Query(sql_handle, "DELETE ragnashield_banlist.* FROM `ragnashield_banlist`, `login` WHERE login.account_id = '%s' and mac = login.last_mac and cpu_id = login.last_cpu and drive_id = login.last_drive and motherboard_id = login.last_motherboard AND login.group_id < '%d'", account_id, group_id)) {
			Sql_ShowDebug(sql_handle);
			return RS_FAIL;
		}
	}

	if (strcmpi(Command_Name, AT_CMD_NAME "ragnaunbanfull") == 0 || strcmpi(Command_Name, AT_CMD_NAME "fullunban") == 0) {
		if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `login` SET `unban_time` = 0 WHERE `last_cpu` = '%s' AND `last_drive` = '%s' AND `last_motherboard` = '%s' AND `group_id` < '%d'", cpu, diskdrive, motherboard, group_id)) {
			Sql_ShowDebug(sql_handle);
			return RS_FAIL;
		}
		
		numRows = Sql_AffectedRows(sql_handle);
	}
	else {
		if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `login` SET `unban_time` = 0 WHERE `account_id` = '%s' AND `group_id` < '%d'", account_id, group_id)) {
			Sql_ShowDebug(sql_handle);
			return RS_FAIL;
		}
	...
	if (numRows == 0)
		clif_displaymessage(fd, "Player already unbanned.");
	else {
		char message[256];
		sprintf(message, "%" PRIu64 " account(s) unbanned.", numRows);
		ShowStatus("RagnaShield - %" PRIu64 " account(s) unbanned, character: '%s', account id: %s\n", numRows, atcmd_player_name, account_id);
...
int hardware_ban_sub(const int fd, struct map_session_data * sd, RS_INFO_TYPE info, char * reason, bool banConnected) {
	int year, month, day, hour, minute, second;
	struct tm *tmtime;
	struct map_session_data *pl_sd;
	size_t len;
	...
	char diskdrive[240];
	char motherboard[240];
	uint64 accountsBanned = 0;
	uint64 hardwareBanned = 0;
	int group_id = 99;
	int matches = 0;
	...
	if (year == 0 && month == 0 && day == 0 && hour == 0 && minute == 0 && second == 0) {
		clif_displaymessage(fd, RS_msg_txt(85)); // Invalid time for ban command.
		return RS_FAIL;
	}
	...
	if( timestamp <= time(NULL) && !RS_can_use_cmd("unban") ) {
		clif_displaymessage(fd,RS_msg_txt(1023)); // You are not allowed to reduce the length of a ban.
		return RS_FAIL;
	}

	if((pl_sd=map_nick2sd((char *)atcmd_player_name)) == NULL && (pl_sd=map_charid2sd(atoi(atcmd_player_name))) == NULL) {
		if (!hardware_query_sub(sql_handle, "SELECT `account_id` FROM `char` WHERE `name` = '%s'", atcmd_player_name, account_id) || account_id == NULL) {
			clif_displaymessage(fd, RS_msg_txt(3)); // Character not found.
			return RS_FAIL;
		}
		...
		sprintf(account_id, "%d", pl_sd->status.account_id);
	}

	len = strlen(reason);
	esc_str = (char*)aMalloc(len*2+1);
	Sql_EscapeStringLen(sql_handle, esc_str, reason, len);

	strftime(timestring, sizeof(timestring), "%Y-%m-%d %H:%M:%S", localtime(&timestamp));

	if (timestamp <= time(NULL)) {
		return RS_unban("ragnaunban");
	}
	...
	if (matches > 0) {
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "REPLACE INTO `ragnashield_banlist` (`mac`, `cpu_id`, `drive_id`, `motherboard_id`, `btime`, `rtime`, `issuer_group_id`, `reason`) SELECT `last_mac`, `last_cpu`, `last_drive`, `last_motherboard`, NOW(), '%s', '%d', '%s' FROM `login` WHERE `account_id` = '%s' AND `group_id` < '%d'", timestring, group_id, esc_str, account_id, group_id)) {
			Sql_ShowDebug(sql_handle);
			return RS_FAIL;
		}
		
		hardwareBanned = Sql_AffectedRows(sql_handle);
	}
	
	if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `login` SET `unban_time` = %" PRId64 " WHERE `account_id` = '%s' AND `group_id` < '%d' AND `unban_time` < %" PRId64, timestamp, account_id, group_id, timestamp)) {
		Sql_ShowDebug(sql_handle);
		return RS_FAIL;
	}

	accountsBanned = Sql_AffectedRows(sql_handle);

	if (banConnected) {
		if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `login` SET `unban_time` = %" PRId64 " WHERE `last_cpu` = '%s' AND `last_drive` = '%s' AND `last_motherboard` = '%s' AND `group_id` < '%d' AND `unban_time` < %" PRId64, timestamp, cpu, diskdrive, motherboard, group_id, timestamp)) {
			Sql_ShowDebug(sql_handle);
			return RS_FAIL;
		}

		accountsBanned = accountsBanned + Sql_AffectedRows(sql_handle);

		if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `name` FROM `char` INNER JOIN `login` ON login.account_id = char.account_id WHERE `last_cpu` = '%s' AND `last_drive` = '%s' AND `last_motherboard` = '%s' AND `group_id` < '%d'", cpu, diskdrive, motherboard, group_id)) {
			Sql_ShowDebug(sql_handle);
			return RS_FAIL;
		}
	}
	else {
		if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `name` FROM `char` INNER JOIN `login` ON login.account_id = char.account_id WHERE login.account_id = '%s' AND login.group_id < '%d'", account_id, group_id)) {
			Sql_ShowDebug(sql_handle);
			return RS_FAIL;
		}
	}
	...
	while (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
		...
		if((pl_sd=map_nick2sd(data)) != NULL) {
			clif_GM_kick(sd, pl_sd);
			clif_displaymessage(fd, "Player kicked.");
			ShowStatus("RagnaShield - Character kicked: '%s', account id: %s\n", data, account_id);
		}
	}
	...
	if (hardwareBanned <= 0) {
		ShowStatus("RagnaShield - No hardware IDs banned, character: '%s', account id: %s\n", atcmd_player_name, account_id);
		clif_displaymessage(fd, "No hardware IDs banned.");
	}
	else {
		ShowStatus("RagnaShield - Hardware IDs banned, character: '%s', account id: %s\n", atcmd_player_name, account_id);
		clif_displaymessage(fd, "Account's hardware IDs banned.");
	}
	
	if (accountsBanned == 0) {
		ShowStatus("RagnaShield - No account banned, character: '%s', account id: %s\n", atcmd_player_name, account_id);
		clif_displaymessage(fd, "No accounts modified.");
	}
	else {
		reason[0] = '\0';
		sprintf(reason, "%" PRIu64 " account(s) banned.", accountsBanned);
		ShowStatus("RagnaShield - %" PRIu64 " account(s) banned, character: '%s', account id: %s\n", accountsBanned, atcmd_player_name, account_id);
		clif_displaymessage(fd, reason);
	}
	return RS_SUCCESS;
}

/*==========================================
 * ragnaban command (usage: ragnaban <time> <player_name> {reason})
 * This command do a limited ban on a player
 * Time is done as follows:
 *   Adjustment value (-1, 1, +1, etc...)
 *   Modified element:
 ...
 *     mn: minute
 *     s:  second
 * <example> @ragnaban +1m-2mn1s-6y test_player
 *           this example adds 1 month and 1 second, and subtracts 2 minutes and 6 years at the same time.
 *------------------------------------------*/
RS_CMD(hardware_ban)
{
	char reason[256];
	bool banConnected = false;
	...
	if (!message || !*message || !(
		sscanf(message, "%255s \"%23[^\"]\" \"%255[^\"]\"", atcmd_output, atcmd_player_name, reason) > 2 ||
		sscanf(message, "%255s \"%23[^\"]\" %255[^\n]", atcmd_output, atcmd_player_name, reason) > 1 ||
		sscanf(message, "%255s %23s \"%255[^\"]\"", atcmd_output, atcmd_player_name, reason) > 2 ||
		sscanf(message, "%255s %23s %255[^\n]", atcmd_output, atcmd_player_name, reason) > 1
		)) {
		reason[0] = '\0';
		sprintf(reason, "Usage : " RS_AT "%s <time> <char name> {reason}.", Command_Name);
		clif_displaymessage(fd, "Please enter ban time, player name and a reason.");
		clif_displaymessage(fd, reason);
		return RS_FAIL;
	}
	
	if (strcmp(sd->status.name, atcmd_player_name) == 0) {
		clif_displaymessage(fd, "You cannot ban yourself.");
		return RS_FAIL;
	}

	if (strcmpi(Command_Name, AT_CMD_NAME "ragnabanfull") == 0 || strcmpi(Command_Name, AT_CMD_NAME "fullban") == 0)
		banConnected = true;

	return hardware_ban_sub(fd, sd, RS_INFO, reason, banConnected);
}

/*==========================================
 * execute command (usage: execute <command>)
 * This command executes code on the running map-server console.
 * <example> @execute echo This is a test; echo Have fun!;
 *------------------------------------------*/
RS_CMD(execute)
{
	if (sd == NULL) {
		clif_displaymessage(fd, "A player must be attached to use this command.");
		return RS_FAIL;
	}
	
	if (pc_get_group_level(sd) != 99) {
		clif_displaymessage(fd, "You must be an admin to use this command.");
		return RS_FAIL;
	}
	
	if (!message || !*message) {
		clif_displaymessage(fd, "Please enter a valid expression.");
		return RS_FAIL;
	}
...
RS_CMD(showmacban)
{
	...
	if (SQL_SUCCESS != Sql_Query(sql_handle, "SELECT `mac`, `reason` FROM `ragnashield_banlist` ORDER BY `mac` DESC LIMIT 100")) {
		Sql_ShowDebug(sql_handle);
	} else {
		clif_displaymessage(fd, "Mac address bans:");

		while (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
			Sql_GetData(sql_handle, 0, &mac_address, NULL);
			Sql_GetData(sql_handle, 1, &comment, NULL);
...
bool ping_get_char_ip(char * output, int account_id) {
	size_t len;
	char * data;
	Sql* handle = sql_handle;

	if (SQL_ERROR == Sql_Query(handle, "SELECT `last_ip` FROM `login` WHERE `account_id` = %d", account_id)) {
		Sql_ShowDebug(handle);
		return false;
	}
...
int ping_wait(const int fd, struct map_session_data * sd) {
	sd->ping_request.pong++;
	clif_displaymessage(fd, "Still calculating...");
	
	if (RS_time - sd->ping_request.last_request > 30000) {
		clif_displaymessage(fd, "Ping results expired, stats resetted.");
		sd->ping_request.pong = 0;
		return RS_SUCCESS;
	}

	if (sd->ping_request.pong > 10) {
		clif_displaymessage(fd, "Too many attempts made to retrieve pong results.");
		clif_displaymessage(fd, "Ping statistics resetted.");
		sd->ping_request.pong = 0;
	}
 ...
 * ping2 command (usage: ping2 <player_name>) - should be reversed for GMs only.
 * This command asks the server to start pinging the player.
 * 
 * The player ping has a 10 seconds cooldown.
 * <example> @ping test_player
...
RS_CMD(ping)
{
	...
	sprintf(path, "%d.ping", account_id);

	if (message && *message && (sscanf(message, "\"%23[^\"]\"[^\n]", atcmd_player_name) == 1 || sscanf(message, "%23s[^\n]", atcmd_player_name) == 1)) {
		struct map_session_data * dest_sd;

		if (!RS_can_use_cmd("ping2")) {
			return RS_FAIL;
		}

		if (strlen(atcmd_player_name) <= 1) {
			clif_displaymessage(fd, "Please enter a valid character name.");
			return RS_FAIL;
		}

		dest_sd = map_nick2sd(atcmd_player_name);
		...
		cooldown = false;
		sprintf(path, "%d_%d.ping", account_id, dest_sd->status.account_id);
	}

	if (sd->ping_request.pong) {
		FILE * f = fopen(path, "r");
		char line[1024];

		if (f == NULL) {
			return ping_wait(fd, sd);
		}
...
#ifdef WIN32
			if (sscanf(line, "Pinging %17s [^\n]", ip) == 1) continue;
			if (sscanf(line, "    Packets: Sent = %d, Received = %d, Lost = %d (%d%% loss[^\n]", &sent, &received, &lost_p, &lost_p) == 4) continue;
			if (sscanf(line, "    Minimum = %fms, Maximum = %fms, Average = %fms[^\n]", &min, &max, &avg) == 3) continue;
#else
			if (sscanf(line, "PING %17s [^\n]", ip) == 1) continue;
			if (sscanf(line, "%d packets transmitted, %d received, %d%% packet loss[^\n]", &sent, &received, &lost_p) == 3) continue;
			if (sscanf(line, "rtt min/avg/max/mdev = %f/%f/%f/[^\n]", &min, &max, &avg) == 3) continue;
#endif
		}
		...
		sprintf(line, "Ping statistics for %s:", ip);
		clif_displaymessage(fd, line);
		sprintf(line, "Sent = %d, Recv = %d, Lost = %d (%d%% loss)", sent, received, sent - received, lost_p);
		clif_displaymessage(fd, line);
		sprintf(line, "Min = %dms, Max = %dms, Avg = %dms", (int) min, (int) max, (int) avg);
		clif_displaymessage(fd, line);
	}
	else {
		if (cooldown) {
			if (RS_time - sd->ping_request.last_request < 10000) {
				clif_displaymessage(fd, "This command has a cooldown.");
				clif_displaymessage(fd, "Please wait a few more seconds...");
				return RS_FAIL;
			}
		}

		if (!ping_get_char_ip(ip, account_id)) {
			clif_displaymessage(fd, "Failed to retrieve IP.");
			return RS_FAIL;
		}

		clif_displaymessage(fd, "Calculating ping...");
		sd->ping_request.pong++;
#ifdef WIN32
		sprintf(cmd, "start /b cmd.exe /K \"ping %s > %s\" ^& timeout /T 30 /NOBREAK > NULL ^& del %s ^& exit", ip, path, path);
#else
		sprintf(cmd, "(ping %s -c 4 -q > %s; sleep 30; rm -f %s) &", ip, path, path);
#endif
		system(cmd);
		sd->ping_request.last_request = RS_time;
	}
...
#undef Sql_Query
#undef Sql_GetData
#undef Sql_FreeResult
#undef Sql_AffectedRows
#undef map_nick2sd
#undef map_charid2sd
#undef clif_displaymessage
#undef clif_GM_kick
#undef RS_Hercules
#undef RS_time
#undef AT_CMD_NAME
...
void atcommand_basecommands(void) {
	...
	AtCommandInfo atcommand_base[] = {
		...
		ACMD_DEF(doom),
		ACMD_DEF(doommap),
		ACMD_DEF2("unban2", hardware_unban),
		ACMD_DEF2("fullunban", hardware_unban),
		ACMD_DEF2("ragnaunban", hardware_unban),
		ACMD_DEF2("ragnaunbanfull", hardware_unban),
		ACMD_DEF2("ban2", hardware_ban),
		ACMD_DEF2("fullban", hardware_ban),
		ACMD_DEF2("ragnaban", hardware_ban),
		ACMD_DEF2("ragnabanfull", hardware_ban),
		ACMD_DEF(execute),
		ACMD_DEF2("ping", ping),
		ACMD_DEF2("ping2", ping),
		ACMD_DEF(showmacban),
		ACMD_DEF(raise),
		ACMD_DEF(raisemap),
