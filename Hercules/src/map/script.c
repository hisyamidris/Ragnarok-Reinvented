...
#define RS_Hercules
#ifdef RS_Hercules
#define RS_BUILDIN			BUILDIN
#define map_nick2sd			map->nick2sd
#define map_id2sd			map->id2sd
#define map_charid2sd		map->charid2sd
#define script_rid2sd		script->rid2sd
...
#ifdef RS_rA
#define RS_BUILDIN			BUILDIN_FUNC
...
RS_BUILDIN(getcharmac) {
	...
	if (sd && sd->fd && session[sd->fd]) {
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `last_mac` FROM `login` WHERE `account_id` = '%d'", sd->status.account_id)) {
		...
		Sql_FreeResult(sql_handle);
		script_pushstrcopy(st, mac);
	}

	return RS_SUCCESS;
}

bool gethardwareid_sub(struct script_state* st, Sql* handle, const char * query, const char * str, char * output) {
	size_t len;
	char *esc_str;
	char * data;
 ...
 * type 4 : Types 1-2-3 combined
 **/
RS_BUILDIN(gethardwareid) {
	struct map_session_data* sd = NULL;
	char account_id[21];
	char hardware_id[60];
	char query[1024];
	...
	if (data_isint(data)) {
		hardware_type = script_getnum(st, 2);
	}
	else {
		ShowError("gethardwareid: invalid type for argument '%d'.\n", 1);
		return RS_FAIL;
	}
	...
	if (script_hasdata(st, 3)) {
		...
		if (data_isstring(data)) {
			if (!gethardwareid_sub(st, sql_handle, "SELECT `account_id` FROM `char` WHERE `name` = '%s'", script_getstr(st,3), account_id) || account_id == NULL) {
				script_pushconststr(st,"");
				return RS_SUCCESS;
			}
		}
		else if (data_isint(data)) {
			if (gethardwareid_sub(st, sql_handle, "SELECT `account_id` FROM `char` WHERE `char_id` = '%s'", script_getstr(st,3), account_id) && account_id != NULL) {
			}
			else {
				const char* str = script_getstr(st,3);
				safestrncpy(account_id, str, 20);
			}
		}
		else {
			ShowError("gethardwareid: invalid type for argument '%d'.\n", 2);
		...
		if (!sd) {
			ShowError("gethardwareid: no player attached to this script.\n");
			return RS_FAIL;
		}

		if (sd && sd->fd && session[sd->fd])
			sprintf(account_id, "%d", sd->status.account_id);
		else {
			ShowError("gethardwareid: no player attached to this script.\n");
			return RS_FAIL;
		}
		...
		ShowError("gethardwareid: invalid number of arguments.\n");
		return RS_FAIL;
	}

	switch (hardware_type) {
		case 0: safestrncpy(hardware_id, "last_mac",		 sizeof(hardware_id)); break;
		case 1: safestrncpy(hardware_id, "last_cpu",		 sizeof(hardware_id)); break;
		case 2: safestrncpy(hardware_id, "last_drive",		 sizeof(hardware_id)); break;
		case 3: safestrncpy(hardware_id, "last_motherboard", sizeof(hardware_id)); break;
		case 4:
			if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `last_cpu`,`last_drive`,`last_motherboard` FROM `login` WHERE `account_id` = '%s'", account_id)) {
				Sql_ShowDebug(sql_handle);
				return RS_FAIL;
			}

			if (SQL_SUCCESS != Sql_NextRow(sql_handle)) {
				ShowDebug("RagnaShield - Account not found.\n");
				Sql_ShowDebug(sql_handle);
				Sql_FreeResult(sql_handle);
				return RS_FAIL;
			}

			Sql_GetData(sql_handle, 0, &cpu, NULL);
			Sql_GetData(sql_handle, 1, &drive, NULL);
			Sql_GetData(sql_handle, 2, &motherboard, NULL);

			if (strlen(cpu) > 0 && strlen(drive) > 0 && strlen(motherboard) > 0) {
				hardware_id[0] = cpu[7];
				hardware_id[1] = cpu[8];
				hardware_id[2] = cpu[19];
				hardware_id[3] = cpu[20];
				hardware_id[4] = cpu[21];
				hardware_id[5] = cpu[22];
				hardware_id[6] = cpu[26];
				hardware_id[7] = cpu[27];

				for (i = 0; i < 8; i++) hardware_id[8 + i] = drive[i];
				for (i = 0; i < 8; i++) hardware_id[16 + i] = motherboard[i];
				hardware_id[23] = '\0';

				Sql_FreeResult(sql_handle);
				script_pushstrcopy(st, hardware_id);
				return RS_SUCCESS;
			}
			else {
				ShowDebug("RagnaShield - The account %s has missing hardware information.\n", account_id);
				Sql_ShowDebug(sql_handle);
				Sql_FreeResult(sql_handle);
				return RS_FAIL;
			}
			...
			break;
		default:
			ShowError("gethardwareid: invalid hardware type '%d'.\n", hardware_type);
			return RS_FAIL;
			break;
	}

	sprintf(query, "SELECT `%s` FROM `login` WHERE `account_id` = '%s'", hardware_id, account_id);

	if (!gethardwareid_sub(st, sql_handle, query, "", hardware_id) || hardware_id == NULL) {
		ShowWarning("gethardwareid: account id '%s' couldn't be found.\n", account_id);
		script_pushconststr(st,"");
		return RS_SUCCESS;
	}
