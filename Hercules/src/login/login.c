...
bool login_fromchar_parse_wrong_pincode(int fd)
{
	...
	if( accounts->load_num(accounts, &acc, RFIFOL(fd,2) ) ) {
		...
		ragna_login_log_acc(&acc, acc.last_ip, acc.userid, 100, "PIN Code check failed"); // FIXME: Do we really want to log this with the same code as successful logins?
	}
...
int login_mmo_auth(struct login_session_data* sd, bool isServer) {
	...
	acc.unban_time = 0;
	safestrncpy(acc.cpu_id, sd->cpu_id, sizeof(acc.cpu_id));
	safestrncpy(acc.drive_id, sd->drive_id, sizeof(acc.drive_id));
	safestrncpy(acc.motherboard_id, sd->motherboard_id, sizeof(acc.motherboard_id));
	safestrncpy(acc.mac, sd->mac, sizeof(acc.mac));
	acc.logincount++;
...
void login_auth_ok(struct login_session_data* sd)
{
	...
	login_log(sd, ip, sd->userid, 100, "login ok");
...
void login_auth_failed(struct login_session_data* sd, int result)
{
	...
	if (login_config.log_login)
	{
		...
		login_log(sd, ip, sd->userid, result, error); // FIXME: result can be 100, conflicting with the value 100 we use for successful login...
	}
...
bool login_parse_client_login(int fd, struct login_session_data* sd, const char *const ip)
{
	...
	if(command == 0x0825)
	{
		...
		size_t uTokenLen = packet_len - RS_PACKET_LENGTH - 0x5C;
	...
	if (!rs_auth_check(sd, ip, username, fd, packet_len)) {
	...
	if (!rs_ban_check(sd, ip, username)) {
		return true;
	}
...
void login_parse_request_connection(int fd, struct login_session_data* sd, const char *const ip, uint32 ipl)
{
	...
	login_log(sd, sockt->session[fd]->client_addr, sd->userid, 100, message);
...
int login_parse_login(int fd)
{
	...
	if( sd == NULL )
	{
		...
		if (login_config.ipban && !sockt->trusted_ip_check(ipl) && ipban_check(ipl))
		{
			...
			login_log(sd, ipl, "unknown", -3, "ip banned");
...
int do_final(void) {
	...
	login_log(NULL, 0, "login server", 100, "login server shutdown");
...
int do_init(int argc, char** argv)
{
	...
	login->RS_CONF_NAME    = aStrdup("conf/ragnashield.conf");
	...
	ragna_config_read(login->RS_CONF_NAME);
	...
	// initialize static and dynamic ragnaban system
	ragnaban_init();
	...
	login_log(NULL, 0, "login server", 100, "login server started");
