...
static bool mmo_auth_fromsql(AccountDB_SQL* db, struct mmo_account* acc, int account_id)
{
	...
	if( SQL_ERROR == SQL->Query(sql_handle,
	    "SELECT `account_id`,`userid`,`user_pass`,`sex`,`email`,`group_id`,`state`,`unban_time`,`expiration_time`,`logincount`,`lastlogin`,`last_ip`,`birthdate`,`character_slots`,`pincode`,`pincode_change`,`last_mac`,`last_cpu`,`last_drive`,`last_motherboard` FROM `%s` WHERE `account_id` = %d",
	...
	SQL->GetData(sql_handle, 16, &data, NULL); safestrncpy(acc->mac, data, sizeof(acc->mac));
	SQL->GetData(sql_handle, 17, &data, NULL); safestrncpy(acc->cpu_id, data, sizeof(acc->cpu_id));
	SQL->GetData(sql_handle, 18, &data, NULL); safestrncpy(acc->drive_id, data, sizeof(acc->drive_id));
	SQL->GetData(sql_handle, 19, &data, NULL); safestrncpy(acc->motherboard_id, data, sizeof(acc->motherboard_id));
...
static bool mmo_auth_tosql(AccountDB_SQL* db, const struct mmo_account* acc, bool is_new)
{
	...
	{// insert into account table
		if( SQL_SUCCESS != SQL->StmtPrepare(stmt,
			"INSERT INTO `%s` (`account_id`, `userid`, `user_pass`, `sex`, `email`, `group_id`, `state`, `unban_time`, `expiration_time`, `logincount`, `lastlogin`, `last_ip`, `birthdate`, `character_slots`, `pincode`, `pincode_change`, `last_mac`, `last_cpu`, `last_drive`, `last_motherboard`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
			db->account_db)
		...
		||  SQL_SUCCESS != SQL->StmtBindParam(stmt, 16, SQLDT_STRING, (void*)&acc->mac,				strlen(acc->mac))
		||  SQL_SUCCESS != SQL->StmtBindParam(stmt, 17, SQLDT_STRING, (void*)&acc->cpu_id,			strlen(acc->cpu_id))
		||  SQL_SUCCESS != SQL->StmtBindParam(stmt, 18, SQLDT_STRING, (void*)&acc->drive_id,		strlen(acc->drive_id))
		||  SQL_SUCCESS != SQL->StmtBindParam(stmt, 19, SQLDT_STRING, (void*)&acc->motherboard_id,  strlen(acc->motherboard_id))
		...
		if( SQL_SUCCESS != SQL->StmtPrepare(stmt, "UPDATE `%s` SET `userid`=?,`user_pass`=?,`sex`=?,`email`=?,`group_id`=?,`state`=?,`unban_time`=?,`expiration_time`=?,`logincount`=?,`lastlogin`=?,`last_ip`=?,`birthdate`=?,`character_slots`=?,`pincode`=?,`pincode_change`=?,`last_mac`=?,`last_cpu`=?,`last_drive`=?,`last_motherboard`=? WHERE `account_id` = '%d'", db->account_db, acc->account_id)
		...
		||  SQL_SUCCESS != SQL->StmtBindParam(stmt, 15, SQLDT_STRING, (void*)&acc->mac,				strlen(acc->mac))
		||  SQL_SUCCESS != SQL->StmtBindParam(stmt, 16, SQLDT_STRING, (void*)&acc->cpu_id,			strlen(acc->cpu_id))
		||  SQL_SUCCESS != SQL->StmtBindParam(stmt, 17, SQLDT_STRING, (void*)&acc->drive_id,		strlen(acc->drive_id))
		||  SQL_SUCCESS != SQL->StmtBindParam(stmt, 18, SQLDT_STRING, (void*)&acc->motherboard_id,  strlen(acc->motherboard_id))
		||  SQL_SUCCESS != SQL->StmtExecute(stmt)
		) {
			SqlStmt_ShowDebug(stmt);
			break;
