...
struct Login_Config {
	...
	bool ragnaban;                                  ///< perform hardware blocking (via contents of `ragnabanlist`) ?
	bool allow_rogue_clients;                       ///< allows clients without RagnaShield to connect to the server ?
	unsigned int ban_sensitivity;					///< determines how precise the hardware IDs must be to allow a ban
