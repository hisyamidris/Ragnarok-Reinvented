...
void login_log(struct login_session_data* sd, uint32 ip, const char* username, int rcode, const char* message)
{
	...
	if (sd == NULL)
		...
		ragna_login_log(sd, ips, username, rcode, message);
}
...
void ragna_login_log_acc(struct mmo_account* acc, const char* ip, const char* username, int rcode, const char* message)
{
	...
	retcode = SQL->Query(sql_handle,
		"INSERT INTO `%s`(`time`,`ip`,`user`,`rcode`,`log`,`mac`,`cpu`,`drive`,`motherboard`) VALUES (NOW(), '%s', '%s', '%d', '%s', '%s', '%s', '%s', '%s')",
		log_login_db, ip, esc_username, rcode, esc_message, acc->mac, acc->cpu_id, acc->drive_id, acc->motherboard_id);
...
void ragna_login_log(struct login_session_data* sd, const char* ip, const char* username, int rcode, const char* message)
{
	...
	retcode = SQL->Query(sql_handle,
		"INSERT INTO `%s`(`time`,`ip`,`user`,`rcode`,`log`,`mac`,`cpu`,`drive`,`motherboard`) VALUES (NOW(), '%s', '%s', '%d', '%s', '%s', '%s', '%s', '%s')",
		log_login_db, ip, esc_username, rcode, esc_message, sd->mac, sd->cpu_id, sd->drive_id, sd->motherboard_id);
